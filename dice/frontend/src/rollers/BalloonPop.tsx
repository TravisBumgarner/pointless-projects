...
const COLORS_ARRAY = [PALETTE.wow.redLight, PALETTE.wow.blueLight, PALETTE.wow.purpleLight, PALETTE.wow.greenLight]
...
const Balloon: React.FC<BalloonProps> = ({ number, isWinner, trigger, sides }) => {
  ...
  return (
    <Box
      sx={{
        ...
        transform: inflating ? 'scale(1.2)' : popped ? 'scale(0.1)' : 'scale(1)',
        opacity: popped ? 0 : 1
      }}
    >
      {popped ? (
...
const BalloonDice: React.FC<DiceRollerProps> = ({ params: { sides }, isRolling, setIsRolling }) => {
